...
import com.example.user.projetoquimicaConsolidados.QuizActivity;
            ...
            public void onClick(View v) {
                if(!etQuantidadePerguntasDiagnostico.getText().toString().equals("")){
                    ...
                    if (spMultiConteudosDiagnostico.getSelectedSize() > 0) {
                        ...
                        Intent it = new Intent(FiltroDiagnosticoActivity.this, QuizActivity.class);
                        it.putExtra("listaConteudos", listaConteudosSelecionados);
                        it.putExtra("quantidade",quantidadePerguntas);
                        it.putExtra("tipo", 2); //1 = quiz(QuizFiltroActivity), 2 = diagnóstico
                        startActivity(it);
